...
export const acceptRide = onCall(async (request: CallableRequest) => {
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "You must be logged in to accept a ride.");
  }
  ...
  if (!rideId) {
    throw new HttpsError("invalid-argument", "The function must be called with a 'rideId'.");
  }
  ...
  try {
    await db.runTransaction(async (transaction) => {
      ...
      if (!rideDoc.exists) {
        throw new HttpsError("not-found", "This ride could not be found.");
      }

      const rideData = rideDoc.data();
      // Check if the ride is still available to be accepted.
      if (rideData?.status !== "pending") {
        throw new HttpsError("failed-precondition", "This ride has already been accepted or is no longer pending.");
      }
      ...
      transaction.update(rideRef, {
        status: "accepted",
        driverId: driverId,
        driverName: request.auth?.token.name || "Driver", // Get name from auth token
    ...
    throw new HttpsError("internal", "An unexpected error occurred while trying to accept the ride.");
  }
});
